...
if is_posix():
    ...
    if os.path.isfile("/etc/ssh/sshd_config"):
        ...
        if is_config_enabled("ROOT_CHECK"):
            ...
            if match:
                ...
                warning += "[!] Issue identified: /etc/ssh/sshd_config allows RootLogin. An attacker can gain root access to the system if password is guessed. Recommendation: Change RootLogin yes to RootLogin no\n\r\n\r"

        match = re.search(r"Port 22\b", data)
        if match and is_config_enabled("SSH_DEFAULT_PORT_CHECK"):
                # trigger warning if match
            warning += "[!] Issue identified: /etc/ssh/sshd_config. SSH is running on the default port 22. An attacker commonly scans for these type of ports. Recommendation: Change the port to something high that doesn't get picked up by typical port scanners.\n\r\n\r"
        ...
        if match:
            warning += "[!] Issue identified: Password authentication enabled. An attacker may be able to brute force weak passwords.\n\r\n\r"

            match = re.search("Protocol 1|Protocol 2,1", data)
        #
        if match:
            # triggered
            warning += "[!] Issue identified: SSH Protocol 1 enabled which is potentially vulnerable to MiTM attacks. https://www.kb.cert.org/vuls/id/684820\n\r\n\r"
    ...
    if os.path.isfile("/etc/vsftpd.conf"):
        ...
        if match:
            # trigger warning if match
            warning += "[!] Issue identified: /etc/vsftpd.conf allows Anonymous login. An attacker can gain a foothold to the system with absolutel zero effort. Recommendation: Change anonymous_enable yes to anonymous_enable no\n\r\n\r"
