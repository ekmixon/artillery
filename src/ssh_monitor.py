...
def ssh_monitor(monitor_frequency):
    ...
    while 1:
        ...
        if os.path.isfile("/var/log/auth.log"):
            ...
            # for OS X
            if os.path.isfile("/var/log/secure.log") and counter == 0:
                fileopen1 = open("/var/log/secure.log", "r")
                counter = 1

        # for centOS
        if os.path.isfile("/var/log/secure") and counter == 0:
            fileopen1 = open("/var/log/secure", "r")
            counter = 1

        # for Debian
        if os.path.isfile("/var/log/faillog") and counter == 0:
            fileopen1 = open("/var/log/faillog", "r")
            counter = 1

        if not os.path.isfile(globals.g_banlist):
            with open(globals.g_banlist, "w") as filewrite:
                filewrite.write("")
        try:
            ...
            for line in fileopen1:
                ...
                line = line.rstrip()
                if match := re.search("Failed password for", line):
                    ssh_counter = ssh_counter + 1
                    line = line.split(" ")
                    # pull ipaddress
                    ipaddress = line[-4]
                    if is_valid_ipv4(ipaddress) and ssh_counter >= int(
                        ssh_attempts
                    ):
                        ...
                        if counter == 0:
                            ...
                            if whitelist_match == 0:
                                ...
                                alert = f"Artillery has blocked (blacklisted) the following IP for SSH brute forcing violations: {ipaddress}"
            ...
            print(f"[*] An error in ssh monitor occured. Printing it out here: {str(e)}")
